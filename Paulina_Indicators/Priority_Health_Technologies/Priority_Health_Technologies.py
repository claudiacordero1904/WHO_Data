...
def find_all_topic_indicators(ind_df):
    search_words = ['Medical devices', 'health technology', 'biomedical', 'MRI', 'CT', 'radiotherapy', 'mammography', 'health infrastructure', 'hospitals density']
...
def save_outputs(topic_long, topic_wide):
    ...
    topic_long.to_csv(outdir / "priority_health_technologies_all_long.csv", index=False)
    topic_wide.to_csv(outdir / "priority_health_technologies_all_wide.csv")
