...
def find_all_topic_indicators(ind_df):
    search_words = ['WASH', 'water', 'sanitation', 'hygiene', 'handwashing', 'open defecation', 'diarrhoea', 'diarrhea', 'unsafe WASH']
...
def save_outputs(topic_long, topic_wide):
    ...
    topic_long.to_csv(outdir / "water_sanitation_and_hygiene_wash_all_long.csv", index=False)
    topic_wide.to_csv(outdir / "water_sanitation_and_hygiene_wash_all_wide.csv")
