...
def find_all_topic_indicators(ind_df):
    search_words = ['Road safety', 'road traffic', 'traffic deaths', 'seat-belt', 'helmet', 'drink-driving', 'BAC']
...
def save_outputs(topic_long, topic_wide):
    ...
    topic_long.to_csv(outdir / "road_safety_all_long.csv", index=False)
    topic_wide.to_csv(outdir / "road_safety_all_wide.csv")
