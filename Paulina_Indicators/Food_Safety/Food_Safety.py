...
def find_all_topic_indicators(ind_df):
    search_words = ['Food safety', 'Foodborne', 'foodborne', 'IHR core capacity on food safety', 'JEE food safety']
...
def save_outputs(topic_long, topic_wide):
    ...
    topic_long.to_csv(outdir / "food_safety_all_long.csv", index=False)
    topic_wide.to_csv(outdir / "food_safety_all_wide.csv")
