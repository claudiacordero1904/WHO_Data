...
def find_all_topic_indicators(ind_df):
    search_words = ['Tuberculosis', 'TB', 'MDR-TB', 'RR-TB', 'XDR-TB', 'TB/HIV']
...
def save_outputs(topic_long, topic_wide):
    ...
    topic_long.to_csv(outdir / "tuberculosis_all_long.csv", index=False)
    topic_wide.to_csv(outdir / "tuberculosis_all_wide.csv")
