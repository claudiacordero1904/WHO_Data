...
def find_all_topic_indicators(ind_df):
    search_words = ['Sexually Transmitted', 'STI', 'syphilis', 'gonorrhoea', 'gonorrhea', 'chlamydia', 'trichomoniasis', 'GASP', 'Neisseria']
...
def save_outputs(topic_long, topic_wide):
    ...
    topic_long.to_csv(outdir / "sexually_transmitted_infections_all_long.csv", index=False)
    topic_wide.to_csv(outdir / "sexually_transmitted_infections_all_wide.csv")
