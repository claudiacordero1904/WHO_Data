...
def find_all_topic_indicators(ind_df):
    search_words = ['Health taxes', 'alcoholic beverages', 'non-alcoholic beverages', 'sugar', 'excise', 'VAT', 'sales tax', 'earmarked']
...
def save_outputs(topic_long, topic_wide):
    ...
    topic_long.to_csv(outdir / "health_taxes_all_long.csv", index=False)
    topic_wide.to_csv(outdir / "health_taxes_all_wide.csv")
