...
def find_all_topic_indicators(ind_df):
    search_words = ['Vaccine-preventable', 'vaccine preventable', 'measles', 'polio', 'diphtheria', 'tetanus', 'pertussis', 'rubella', 'mumps', 'yellow fever', 'rotavirus', 'BCG', 'HPV']
...
def save_outputs(topic_long, topic_wide):
    ...
    topic_long.to_csv(outdir / "vaccine_preventable_diseases_all_long.csv", index=False)
    topic_wide.to_csv(outdir / "vaccine_preventable_diseases_all_wide.csv")
